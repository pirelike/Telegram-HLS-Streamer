...
from src.storage.database import DatabaseManager, SegmentInfo, VideoInfo, SubtitleInfo, SubtitleFileInfo
from src.utils.logging import logger
from datetime import datetime, timezone
from src.utils.networking import calculate_file_hash, format_file_size
...
class RoundRobinTelegramHandler:
    ...
    async def upload_file(self, file_path: str) -> dict:
        """Upload a single file using round-robin bot selection."""
        if not self.bots:
            raise Exception("No bots available for upload")
        
        # Select bot using round-robin (simple modulo for now)
        import hashlib
        file_hash = hashlib.md5(file_path.encode()).hexdigest()
        bot_index = int(file_hash, 16) % len(self.bots)
        bot = self.bots[bot_index]
        
        file_size = os.path.getsize(file_path)
        if file_size > self.upload_limit:
            raise Exception(f"File too large: {file_size} bytes > {self.upload_limit} bytes")
        ...
        try:
            ...
            # Return file info similar to Telegram API format
            return {
            ...
            logger.error(f"Failed to upload {file_path} with {bot['id']}: {e}")
