...
## 5. Web App Backend (app/main.py)This is the main application file containing all the server-side logic.import os
    ...
    def get_file_mapping(file_unique_id: str) -> str | None:
        """Get file_id from database."""
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute('SELECT file_id FROM file_mappings WHERE file_unique_id = ?', (file_unique_id,))
    ...
    async def upload_file_with_progress(application, chat_id: str, file_path: Path, task_id: str):
        """Upload file with chunked reading and progress tracking."""
            ...
            for i, segment_path in enumerate(segments):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        # Store the file mapping in database and memory
    ...
    async def stream_handler(bot_token: str, file_unique_id: str, filename: str):
        ...
        file_id = file_mappings.get(file_unique_id)
    
        # If not in memory, try database
        if not file_id:
            file_id = get_file_mapping(file_unique_id)
            if file_id:
        ...
        if not file_id:
            async with ClientSession() as session:
                ...
                if not file_id:
                    raise HTTPException(status_code=404, detail="File not found. It may have been uploaded too long ago or the mapping was lost.")
                # Store it for future use
                file_mappings[file_unique_id] = file_id
                store_file_mapping(file_unique_id, file_id, filename, bot_token)
